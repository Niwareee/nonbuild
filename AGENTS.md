...
│   └── DeploymentMap.java   Carte ASCII de /deploy map (classe PURE, testable, sans
│                            Bukkit) : le cadre et l'échelle suivent les ARÈNES COLLÉES
│                            (corner1/2), jamais les cellules réservées — fidèle au monde
│                            réel. LETTRES = volume collé (jamais invisible, même sous-pixel),
│                            + spawn, @ joueur, contour ░ zone protégée et bordure · cellule
│                            en arrière-plan discret (clippés au cadre). Échelle fine sans
│                            plancher (peut descendre sous 16 blocs/case). Deux rendus :
│                            render (vue d'ensemble compacte) et renderZoom (une instance en
│                            forme, contexte 32 blocs). Légende par instance avec coordonnées
│                            exactes du volume collé.
├── schematic/
...
| `/deploy map` ou `/deploy map <instance>`                                | Carte ASCII fidèle au monde (cadre sur les arènes collées, échelle fine, zone protégée + cellules en arrière-plan discret, légende avec coordonnées exactes). Sans arg = vue d'ensemble ; avec `<instance>` = zoom sur cette instance (arène en forme). + stats.                                                                                                                                                                                                                          |
